...
export default function Problems() {
  ...
  const getData = async () => {
    ...
    await axiosAPI.get(endpoints.problems + "/get-by-admin?" + params.toString()).then((res) => {
  ...
  const handleDeleteProblemWithoutAuthor = async () => {
    const res = await axiosAPI.get(endpoints.problems + "/get-problems-without-author");
    ...
    if (cf) {
      const cf2 = await confirm("Delete!", "This action can't Ctrl Z? Folder will be delete forever! Are you sure?");
      if (cf2)
        await axiosAPI.delete(endpoints.problems + "/delete-problem-without-author").then((res) => {
          res.status === 200 && toast.success(`Delete ${res.data?.length} problem Successfully!`);
        });
    }
  };
            ...
            {data.length > 0 ? (
              data.map((row) => (
                ...
                >
                  ...
                  <TableCell align='center'>{row.author?._id || "---"}</TableCell>
                  <TableCell align='center'>{row.uuid || "---"}</TableCell>
                  <TableCell align='center'>{row.title || "---"}</TableCell>
                  <TableCell align='center'>{row.slug || "---"}</TableCell>
                  <TableCell align='center'>
                    <Stack justifyContent='center' direction='row' spacing={1}>
                      <Button size='small' variant='outlined' onClick={() => handleDetails(row._id)}>
                        DETAILS
                      </Button>
                      <Button
                        size='small'
