...
export default function Problems() {
  ...
  const getData = async () => {
    ...
    await axiosAPI.get(endpoints.submissions + "/get-by-admin?" + params.toString()).then((res) => {
  ...
  const handleDeleteSubmissionsWithoutAuthorOrProblem = async () => {
    const res = await axiosAPI.get(endpoints.submissions + "/get-submissions-without-author-without-problem");
    ...
    if (cf) {
      ...
      if (cf2)
        await axiosAPI
          .delete(endpoints.submissions + "/delete-submissions-without-author-without-problem")
          .then((res) => {
            res.status === 200 && toast.success(`Delete ${res.data?.length} submissions successfully!`);
          });
    }
  };
