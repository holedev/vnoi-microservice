...
export default function Dashboard() {
  ...
  const [filter, setFiliter] = useState({
    limit:
      parseInt(searchParams.get('limit')) || import.meta.env.VITE_PAGE_SIZE,
  ...
  const handleDelete = async (_id) => {
    ...
    if (isConfirmed) {
      await axiosAPI
        .patch(endpoints.learning + `/courses/${_id}`)
        .then((res) => {
          if (res.status === 200) {
            setData((prev) => {
              ...
              {loading ? (
                ...
                data.map((row) => (
                  ...
                  >
                    ...
                    <TableCell align="center">
                      <Button
                        onClick={() => nav('/lecturer/courses/' + row._id)}
                        size="small"
                        variant="contained"
                      >
                        Edit
